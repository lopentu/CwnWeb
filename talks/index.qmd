...
![](llm_freq.png){width=20%}

- 數據也有可能受到版權、個資與企業隱私問題。


## in-context learning and limitations
`In-context learning (~prompting) `

- involves providing input to the language model in a specific format to elicit the desired output.
context window restricts the model’s ability to process long sequences of information effectively.
...
- Retrieval-augmented in-context learning (e.g., RAG prompting)
...
- 商用好，但很貴，也不保證安全。
- 開源的 llm 越來越好，壓縮技術越見成熟。但訓練不便宜，動輒政治價值審查。
...
# CWN 2.0 and SoMe Corpus plugins for LLMs
[lopeGPT]{#magenta}: a RAG model high-level architecture
[lopeGPT](https://lope.linguistics.ntu.edu.tw/lopeGPT) 
